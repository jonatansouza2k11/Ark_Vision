...
import UserBulkActions from '../components/users/UserBulkActions'; // ➕ OPÇÃO 2
...
export default function Users() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      alert(
        `✅ Operação concluída!\n\n` +
        `Deletados: ${response.summary.successful}\n` +
        `Falhas: ${response.summary.failed}`
      );
    } catch (error) {
      console.error('Erro ao deletar usuários em lote:', error);
      alert('❌ Erro ao deletar usuários. Verifique o console para detalhes.');
    } finally {
  ...
  const handleExport = async (format: 'json' | 'csv') => {
    try {
      ...
      alert(`✅ Usuários exportados com sucesso em formato ${format.toUpperCase()}!`);
    } catch (error) {
      console.error('Erro ao exportar usuários:', error);
      alert('❌ Erro ao exportar usuários. Verifique o console para detalhes.');
    } finally {
  ...
  return (
    <MainLayout>
      <div className="space-y-6 animate-fadeIn">
        ...
        {/* ========== BULK ACTIONS BAR - OPÇÃO 2 (COMPONENTE SEPARADO) ========== */}
        ...
        {/* ========== BULK ACTIONS BAR - OPÇÃO 1 (INLINE - COMENTADO) ==========
        {isBulkMode && selectedUserIds.length > 0 && (
          <div className="bg-purple-50 border-2 border-purple-200 rounded-lg p-4 flex items-center justify-between animate-fadeIn">
            <div className="flex items-center gap-4">
              <span className="text-purple-900 font-semibold">{selectedUserIds.length} usuário(s) selecionado(s)</span>
              <button onClick={handleSelectAll} className="text-sm text-purple-600 hover:text-purple-800 underline">
                {selectedUserIds.length === filteredUsers.length ? 'Desselecionar Todos' : 'Selecionar Todos'}
              </button>
            </div>
            ...
            <div className="flex gap-2">
              ...
              >
                {bulkDeleting ? '⏳ Deletando...' : '🗑️ Deletar Selecionados'}
              </button>
              <button
